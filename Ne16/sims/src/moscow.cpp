...
bool moscow::getEvent8C(float Etotal,CFrame * Frag1,CFrame *Frag2,CFrame *Frag3)
{
  ...
  float p3[3];
    ...
    for (int i=0;i<3;i++) 
      {
        Frag1->v[i] = p1[i]/Frag1->mass/constant;
        Frag2->v[i] = p2[i]/Frag2->mass/constant;
        Frag3->v[i] = p3[i]/Frag3->mass/constant;
        tot += pow(Frag1->v[i]/c,2)/2.*Frag1->mass*amu;
        tot += pow(Frag2->v[i]/c,2)/2.*Frag2->mass*amu;
        tot += pow(Frag3->v[i]/c,2)/2.*Frag3->mass*amu;
      }
